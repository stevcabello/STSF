...
%    
%    if i == 1
%     h = text(1.2, 100 - 5*(n+1)- 10*i + 2, num2str(r(i)), 'FontSize', 14, 'HorizontalAlignment', 'right', 'FontWeight', 'bold');
%     text(1.25, 100 - 5*(n+1) - 10*i, labels{idx(i)}, 'FontSize', 16, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left', 'FontWeight', 'bold');
%    else
%     h = text(1.2, 100 - 5*(n+1)- 10*i + 2, num2str(r(i)), 'FontSize', 12, 'HorizontalAlignment', 'right');
%     text(1.25, 100 - 5*(n+1) - 10*i, labels{idx(i)}, 'FontSize', 14, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left');
%    end
% 
% end
% 
...
% 
%     text(-0.2, 100 - 5*(n+1) -10*(k-i+1)+2, num2str(r(i)), 'FontSize', 12, 'HorizontalAlignment', 'left');
%     text(-0.25, 100 - 5*(n+1) -10*(k-i+1), labels{idx(i)}, 'FontSize', 14, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'right');
% 
% end

for i=1:ceil(k/2)

   line([(k-r(i))/(k-1) (k-r(i))/(k-1) 1.2], [100 100-5*(n+1)-10*i 100-5*(n+1)-10*i],'LineWidth', 1.5, 'Color', 'k');
   h = text(1.2, 100 - 5*(n+1)- 10*i + 3, num2str(r(i)), 'FontSize', 13, 'HorizontalAlignment', 'right');

   if i==4 %%this is our approach STSF
       text(1.25, 100 - 5*(n+1) - 10*i, labels{idx(i)}, 'FontSize', 17, 'FontWeight','bold', 'Color', 'b' ,'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left');
   else
       text(1.25, 100 - 5*(n+1) - 10*i, labels{idx(i)}, 'FontSize', 17, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left');
...
for i=ceil(k/2):k
   ...
   line([(k-r(i))/(k-1) (k-r(i))/(k-1) -0.2], [100 100-5*(n+1)-10*(k-i+1) 100-5*(n+1)-10*(k-i+1)], 'LineWidth', 1.5,'Color', 'k');

   text(-0.2, 100 - 5*(n+1) -10*(k-i+1)+3, num2str(r(i)), 'FontSize', 13, 'HorizontalAlignment', 'left');

   text(-0.25, 100 - 5*(n+1) -10*(k-i+1), labels{idx(i)}, 'FontSize', 17, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'right');
...
% axis xy 
% tics = repmat((0:(k-1))/(k-1), 3, 1);
% line(tics(:), repmat([100, 105, 100], 1, k), 'LineWidth', 5, 'Color', 'k');
% tics = repmat(((0:(k-2))/(k-1)) + 0.5/(k-1), 3, 1);
% line(tics(:), repmat([100, 102.5, 100], 1, k-1), 'LineWidth', 5, 'Color', 'k');
% line([0 0 0 cd/(k-1) cd/(k-1) cd/(k-1)], [127 123 125 125 123 127], 'LineWidth', 5, 'Color', 'k');
% h = text(0.5*cd/(k-1), 130, 'CD', 'FontSize', 20, 'HorizontalAlignment', 'center');
% 
% for i=1:k
% % 'FontSize', 12,%original size
%    text((i-1)/(k-1), 110, num2str(k-i+1), 'FontSize', 25, 'HorizontalAlignment', 'center');
% 
% end
% 
% % compute average ranks
...
% for i=1:ceil(k/2)
% 
%    line([(k-r(i))/(k-1) (k-r(i))/(k-1) 1.2], [100 100-5*(n+1)-10*i 100-5*(n+1)-10*i],'LineWidth', 3, 'Color', 'k');
%    h = text(1.2, 100 - 5*(n+1)- 10*i + 3, num2str(r(i)), 'FontSize', 30, 'HorizontalAlignment', 'right');
% 
%    if i==3 %%this is our approach STSF
%        text(1.25, 100 - 5*(n+1) - 10*i, labels{idx(i)}, 'FontSize', 40, 'FontWeight','bold', 'Color', 'b' ,'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left');
%    else
%        text(1.25, 100 - 5*(n+1) - 10*i, labels{idx(i)}, 'FontSize', 40, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left');
%        
%    end
%    
% 
...
% % labels displayed on the left
% 
% for i=ceil(k/2)+1:k%%%%the +1 was added by me 9/Jun/2020
%    
%    line([(k-r(i))/(k-1) (k-r(i))/(k-1) -0.2], [100 100-5*(n+1)-10*(k-i+1) 100-5*(n+1)-10*(k-i+1)], 'LineWidth', 3,'Color', 'k');
% 
%    text(-0.2, 100 - 5*(n+1) -10*(k-i+1)+3, num2str(r(i)), 'FontSize', 30, 'HorizontalAlignment', 'left');
%    text(-0.25, 100 - 5*(n+1) -10*(k-i+1), labels{idx(i)}, 'FontSize', 40, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'right');
% 
% end
% 
% % group cliques of statistically similar classifiers
